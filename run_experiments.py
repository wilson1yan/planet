...
def worker(gpu_id, max_per_gpu, exps):
    ...
    processes = []
    for exp in exps:
        method, env, run_id, div_scale, bs = exp
        args = f"python -m planet.scripts.train --logdir logs/cpc_{method}_divscale{div_scale}_bs{'_'.join(bs.split(' '))}/{env}/run_{run_id} " + \
                f"--params '{{tasks: [{env}], model: {method}, divergence_scale: {div_scale}, stack_obs: True, n_stack_history: 4, batch_shape: {bs}}}'"
        print('Running', args)
